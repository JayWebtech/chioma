...
import { ChevronDown, Search, Filter, Loader2, Eye, Download, Trash2, Archive, FileText, FolderOpen } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import { useLandlordDocuments, DocumentStatus, DocumentType, DocumentRecord, useDeleteDocument, useArchiveDocument } from '@/lib/query/hooks/use-landlord-documents';
...
export function DocumentsList({ className = '', onViewDocument }: DocumentsListProps) {
    ...
    const { data: documents = [], isLoading, error } = useLandlordDocuments({
    ...
    const getTypeBadge = (type: DocumentType) => {
        const config = {
            LEASE: { className: 'bg-purple-100 text-purple-700 border-purple-200', label: 'Lease' },
            INSPECTION: { className: 'bg-yellow-100 text-yellow-700 border-yellow-200', label: 'Inspection' },
            RECEIPT: { className: 'bg-green-100 text-green-700 border-green-200', label: 'Receipt' },
            CONTRACT: { className: 'bg-blue-100 text-blue-700 border-blue-200', label: 'Contract' },
    ...
    const columns = React.useMemo<ColumnDef<DocumentRecord>[]>(
        () => [
                ...
                cell: ({ row }) => (
                    <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <div className="font-medium text-neutral-900 truncate max-w-xs" title={row.getValue('name')}>
                                {row.getValue('name')}
                            </div>
            ...
            },
            {
                ...
                cell: ({ row }) => (
                    <div className="text-sm text-neutral-500">
                        {formatDistanceToNow(new Date(row.getValue('uploadedAt')), { addSuffix: true })}
                    </div>
                ),
                sortingFn: 'datetime',
            },
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed border-neutral-200 rounded-3xl">
                ...
                <h3 className="text-lg font-semibold text-neutral-900 mb-2">Failed to load documents</h3>
                <p className="text-neutral-500 mb-6 max-w-sm">There was an issue fetching your documents. Please refresh the page.</p>
            ...
            <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-neutral-100 overflow-hidden">
                {isLoading ? (
                    ...
                    <div className="p-16 text-center border-2 border-dashed border-neutral-200 rounded-3xl">
                        ...
                        <p className="text-neutral-500 mb-6">Upload your first document to get started.</p>
                    </div>
                ) : (
                    <>
